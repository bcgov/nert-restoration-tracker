...
import { POST, createPlan } from './create';
...
describe('create', () => {
  ...
  describe('createPlan', () => {
    ...
    it('creates a new project', async () => {
      ...
      sinon.stub(PlanService.prototype, 'createPlan').resolves({ id: 1 } as any);

      const { mockReq, mockRes, mockNext } = getRequestHandlerMocks();

      try {
        const requestHandler = createPlan();
    ...
    it('catches error, calls rollback, and re-throws error', async () => {
      ...
      sinon.stub(PlanService.prototype, 'createPlan').rejects(new Error('a test error'));
