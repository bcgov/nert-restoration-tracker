...
const useProjectParticipationApi = (axios: AxiosInstance) => {
  ...
  const removeProjectParticipant = async (
    ...
    const { status } = await axios.delete(
      `/api/project/${projectId}/participants/${projectParticipationId}`
    );
   ...
   */
  const getUserProjectParticipant = async (
    projectId: number
  ): Promise<IGetUserProjectParticipantResponse> => {
    const { data } = await axios.get<IGetUserProjectParticipantResponse>(
      `/api/project/${projectId}/participants/self`
    );
