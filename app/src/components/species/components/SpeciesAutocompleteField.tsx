...
const SpeciesAutocompleteField = (props: ISpeciesAutocompleteFieldProps) => {
  ...
  const [inputValue, setInputValue] = useState<string>(
    isDefaultSpecies ? defaultSpecies?.scientificName : ''
  );
  // The array of options to choose from
  const [options, setOptions] = useState<(ITaxonomy | IPartialTaxonomy)[]>(
    isDefaultSpecies ? [defaultSpecies] : []
  );
  ...
  const handleSearch = useMemo(
    () =>
      debounce(async (inputValue: string, callback: (searchedValues: ITaxonomy[]) => void) => {
        ...
        const response = await nertApi.taxonomy.searchSpeciesByTerms(searchTerms).catch(() => {
          return [];
        });
  ...
  return (
    <Autocomplete
      ...
      onChange={(_, option) => {
        ...
        setInputValue(
          startCase(option?.commonNames?.length ? option.commonNames[0] : option.scientificName)
        );
      }}
