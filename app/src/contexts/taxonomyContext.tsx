...
export const TaxonomyContextProvider = (props: PropsWithChildren) => {
  ...
  const cacheSpeciesTaxonomyByIds = useCallback(
    async (tsns: number[]) => {
      const fetchTaxonomiesPromise = nertApi.taxonomy
  ...
  return (
    <TaxonomyContext.Provider value={taxonomyContext}>{props.children}</TaxonomyContext.Provider>
  );
};
